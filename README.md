...
- `cert.pem` and `key.pem`: Cloudflare Dashboard -> SSL/TLS -> Origin Server -> Create Certificate (hostname should match `QBIT_DOMAIN`).
...
- `cert.pem` and `key.pem` for qBittorrent HTTPS (Cloudflare Origin Cert for `QBIT_DOMAIN`)
- `jellyfin.pfx` for Jellyfin HTTPS

`jellyfin.pfx` can be created from `cert.pem` + `key.pem`:
...
- `QBIT_DOMAIN` -> `https://QBITTORRENT_IP:QBIT_WEBUI_PORT`
...
Check qBittorrent is using HTTPS and managed security keys:

```bash
docker compose exec -T qbittorrent sh -lc 'sed -n "1,220p" /config/config/qBittorrent.conf'
```
